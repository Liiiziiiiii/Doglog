...
export default function AddDogTreeElement({ obj = { name: "", photo: "" }, onUpdate, name, isFilled }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (storedUserId) {
                try {
                    const response = await axios.get(`https://cdq2m359-5254.euw.devtunnels.ms/api/DogDetails/users-with-dogs/${storedUserId}`);
    ...
    const handleSelectSuggestion = async (suggestion) => {
        ...
        try {
            const response = await axios.get(`https://cdq2m359-5254.euw.devtunnels.ms/api/DogDetails/dog-with-ancestors/${suggestion.id}`);
            if (response.data) {
                const ancestorsData = {
